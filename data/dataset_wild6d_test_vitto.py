...
class Wild6DTestDataset(Dataset):
    ...
    def __init__(self, opts):
        ...
        for list_idx, seqname in enumerate(self.test_list):
            ...
            prefix = opts.test_dataset_path.rfind("test")
            
            # my change
            for char in opts.test_dataset_path[prefix:]:
                prefix +=1
                if char == "/":
                    break
    ...
    def __getitem__(self, index):
        ...
        if np.unique(indices[0]).size > 2 and np.unique(indices[1]).size > 2:
            xid = indices[1]
            yid = indices[0]
        else:
            # print("HERE INDICES")
            # print(indices)
            # print(self.imglist[video_id][frame_id])
            xid = np.array([0, img.shape[1]])
            yid = np.array([0, img.shape[0]])
        ...
        pp = [intr[0, 2], intr[1, 2]] # principle point 

        maxw = self.opts.img_size
        maxh = self.opts.img_size


        # # print("length[0]\t",length[0],"\tlength[1]\t",length[1])

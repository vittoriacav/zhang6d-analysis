...
from model.module.network.shape_predictor_vitto import ShapePredictor
...
class Encoder(nn.Module):
    ...
    def forward(self, img, mean_v, pp_crop, foc_crop):
        ...
        if self.opts.codedim != 512:
            shape_code = self.shape_code_predictor(img_code)  # b,c
        else:
            print("Keeping last 512-D feature vector as shape code")
            shape_code = img_code
        pred_v = self.shape_predictor(mean_v, shape_code, rotation)
        ...
        if not self.opts.train:
            return img_feat, mesh_feat, pred_v, rotation, translation, scale, img_code, shape_code
        else:

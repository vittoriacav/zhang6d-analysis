...
class CanonicalMesh(nn.Module):
    ...
    def init_shape(self):
        opts = self.opts
        if opts.shape_prior:
            prior = trimesh.load_mesh(opts.shape_prior_path) # load the prior mesh stored in self-corr-pose/config/*_wild6d/*.obj
            verts = torch.tensor(np.array(prior.vertices)).float()
            faces = torch.tensor(np.array(prior.faces))
            # here he's normalizing the vertices so that the mesh stay is in the unit volume (?)
            verts -= verts.mean(0)
            verts /= verts.abs().max()
            # multiply for the initial scale which is given in input, default is still the unit

...
from data.dataloader_vitto import data_loader
...
class Trainer:
    ...
    def write_log(self, data, total_loss, grad, aux_output, step):
        if self.opts.local_rank <= 0:
            ...
            if (step+1) % self.opts.vis_freq2 == 0:
                enc_dict = {}
                # print("S FROM ENCODER LOG", aux_output['S_encoder'].shape, aux_output['S_encoder'])

                one_rot_mat = R.from_matrix(np.array(aux_output['R_encoder'][0].tolist()))
                one_rot_euler = one_rot_mat.as_euler('xyz', degrees=True)

                if 'R_encoder' in aux_output.keys(): 
                    enc_dict['econder/R_encX'] = one_rot_euler[0]
                    enc_dict['econder/R_encY'] = one_rot_euler[1]
                    enc_dict['econder/R_encZ'] = one_rot_euler[2]
                    
                if 't_encoder' in aux_output.keys(): 
                    enc_dict['econder/t_encX'] = aux_output['t_encoder'][0][0][0]
                    enc_dict['econder/t_encY'] = aux_output['t_encoder'][0][0][1]
                    enc_dict['econder/t_encZ'] = aux_output['t_encoder'][0][0][2]
                
                # print("\nROTATION and TRANSLATION FED to TENSORBOARD")
                # print(one_rot_euler, aux_output['t_encoder'][0][0][0])
                add_scalar(self.log, enc_dict, step)
